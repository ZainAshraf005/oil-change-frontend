...
export default function AssignPackageForm() {
  ...
  useEffect(() => {
    ...
    if (shop?.package_assignments) {
      setSelectedPackage(Number(shop.package_assignments.package.id));
      setStartDate(
        new Date(shop.package_assignments.start_at).toISOString().split("T")[0]
      );
    } else {
      setSelectedPackage(null);
      setStartDate("");
        ...
        prev.map((shop) =>
          shop.id === selectedShop
            ? {
                ...shop,
                package_assignments: {
                  id: shop.package_assignments?.id ?? Date.now().toString(), // temporary or existing id
                  package:
                    packages.find(
        ...
        <div>
          ...
          <div className="text-sm">
            {loading
              ...
              ? `${shop.package_assignments.package.name} (since ${
                  new Date(shop.package_assignments.start_at)
                    .toISOString()
                    .split("T")[0]
                })`
              : "None"}
